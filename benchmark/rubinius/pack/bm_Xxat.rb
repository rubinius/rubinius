...
Benchmark.bmbm do |x|
  ...
  x.report("[S].pack('NX')") do
    d = "NX" * S.size
    total.times { S.pack(d) }
  end

  x.report("[S].pack('Cx')") do
    d = "Cx" * S.size
    total.times { S.pack(d) }
  end

  x.report("[S].pack('N@<n>')") do
    d = (1..S.size).inject("") { |s, x| s << "N@#{x}" }
    total.times { S.pack(d) }
  end

  x.report("[M].pack('NX')") do
    d = "NX" * M.size
    total.times { M.pack(d) }
  end

  x.report("[M].pack('Cx')") do
    d = "Cx" * M.size
    total.times { M.pack(d) }
  end

  x.report("[M].pack('N@<n>')") do
    d = (1..M.size).inject("") { |s, x| s << "N@#{x}" }
    total.times { M.pack(d) }
  end

  x.report("[L].pack('NX')") do
    d = "NX" * L.size
    total.times { L.pack(d) }
  end

  x.report("[L].pack('Cx')") do
    d = "Cx" * L.size
    total.times { L.pack(d) }
  end

  x.report("[L].pack('N@<n>')") do
    d = (1..L.size).inject("") { |s, x| s << "N@#{x}" }
    total.times { L.pack(d) }
  end

  x.report("[X].pack('NX')") do
    d = "NX" * X.size
    total.times { X.pack(d) }
  end

  x.report("[X].pack('Cx')") do
    d = "Cx" * X.size
    total.times { X.pack(d) }
  end

  x.report("[X].pack('N@<n>')") do
    d = (1..X.size).inject("") { |s, x| s << "N@#{x}" }
    total.times { X.pack(d) }
  end
end
