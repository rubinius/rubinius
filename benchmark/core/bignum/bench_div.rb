...
Benchmark.ips do |x|
  ...
  x.report "medium Bignum / small Bignum" do |times|
    i = 0
    while i < times
      medium_bignum / small_bignum
      i += 1
    end
  end

  x.report "large Bignum / medium Bignum" do |times|
    i = 0
    while i < times
      large_bignum / medium_bignum
      i += 1
    end
  end

  x.report "Bignum / Fixnum => Bignum" do |times|
    i = 0
    while i < times
  ...
  x.report "Bignum / Fixnum => Fixnum" do |times|
    i = 0
    while i < times
      small_bignum / fixnum
      i += 1
