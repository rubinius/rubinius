module Rubinius
  class Cuckoo < Hash
    ...
    def find_entry(key)
      ...
      if bucket = @entries[key_index1(key_hash)]
        if (entry = bucket[0] and entry.match? key, key_hash) or
           (entry = bucket[1] and entry.match? key, key_hash)
          return entry
        end
      end

      if bucket = @entries[key_index2(key_hash)]
        if (entry = bucket[0] and entry.match? key, key_hash) or
           (entry = bucket[1] and entry.match? key, key_hash)
          return entry
        end
    ...
    def insert(new_entry)
      ...
      while i < @max_loop
        ...
        unless bucket = @entries[index]
          bucket = Rubinius::Tuple.new @bucket_size
          bucket[0] = new_entry
        ...
        # Search the bucket for a slot
        unless entry = bucket[1]
          bucket[1] = new_entry
          @size += 1
          return
        end
    ...
    def new_entry(bucket, index, key, key_hash, value)
      if key.kind_of? String
        key = key.dup
        key.freeze
      end

      bucket[index] = Entry.new key, key_hash, value
    end
    private :new_entry
    ...
    def []=(key, value)
      ...
      if bucket2 = @entries[index2]
        if (entry2 = bucket2[0] and entry2.match? key, key_hash) or
           (entry2 = bucket2[1] and entry2.match? key, key_hash)
          return entry2.value = value
        end
      end

      # Insert a new bucket in the first table.
      unless bucket
        bucket = Rubinius::Tuple.new @bucket_size
        new_entry bucket, 0, key, key_hash, value
        @entries[index] = bucket
        @size += 1
        return value
      end

      # Bucket already exists, look for an empty slot.
      unless entry = bucket[0]
        new_entry bucket, 0, key, key_hash, value
        @size += 1
        return value
      end

      # First slot was taken, try to insert in slot two before taking over.
      unless bucket[1]
        new_entry bucket, 1, key, key_hash, value
        @size += 1
        return value
      end

      # Both slots are filled, boot one.
      new_entry bucket, 0, key, key_hash, value

      # Attempt to insert nestless entry in the second table.
      unless bucket2
        bucket = Rubinius::Tuple.new @bucket_size
        bucket[0] = entry
        @entries[index] = bucket
      ...
      # Attempt to insert in the first slot.
      unless entry2 = bucket2[0]
        bucket2[0] = entry
        @size += 1
        return value
      end

      # Attemt to insert in the second slot.
      unless entry2 = bucket2[1]
        bucket2[1] = entry
        @size += 1
        return value
      end

      # Both were occupied, boot the second and take over.
      bucket2[1] = entry
    ...
    def delete(key)
      ...
      index = key_index1(key_hash)
      if bucket = @entries[index]
        if entry = bucket[0] and entry.match? key, key_hash
          if bucket[1]
            bucket[0] = nil
          else
            @entries[index] = nil
        ...
        if entry = bucket[1] and entry.match? key, key_hash
          if bucket[0]
            bucket[1] = nil
          else
            @entries[index] = nil
