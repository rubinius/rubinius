...
context "Shotgun" do
  ...
  specify "should convert a string expansion to an sexp" do
    example(@alias) do
      'a = 1; "hello #{a}, you rock."'.to_sexp
    end.should == [:block, [:lasgn, :a, 0, [:lit, 1]], [:dstr, "hello ", [:evstr, [:lvar, :a, 0]], [:str, ", you rock."]]]
  end

  specify "should convert a pathological string expansion to an sexp" do
    example(@alias) do
      '@thing = 5; "hello #@thing, you are crazy."'.to_sexp
    end.should == [:block, [:iasgn, :@thing, [:lit, 5]], [:dstr, "hello ", [:evstr, [:ivar, :@thing]], [:str, ", you are crazy."]]]
  end

  specify "should convert a method definition without arguments to an sexp" do
    example(@alias) do
      "def name; 1; end".to_sexp
    end.should == [:defn, :name, [:scope, [:block, [:args], [:lit, 1]], []]]
  end

  specify "should convert a method definition with arguments to an sexp" do
    example(@alias) do
      "def name(a, b); 1; end".to_sexp
    end.should == [:defn, :name, [:scope, [:block, [:args, [:a, :b], [], nil, nil], [:lit, 1]], [:a, :b]]]
  end

  specify "should convert a class definition to an sexp" do
    example(@alias) do
      "class Blah < A::B; end".to_sexp
    end.should == [:class, [:colon2, :Blah], [:colon2, [:const, :A], :B], [:scope, []]]
  end
  ...
  specify "should convert a heredoc to an sexp" do
    example(@alias) do
      ...
      BLAH
      ".to_sexp
    end.should == [:lasgn, :a, 0, [:str, "      hello\n"]]
  end
end
