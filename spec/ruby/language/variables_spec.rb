...
describe "Multiple assignment" do
  context "with a single RHS value" do
    ...
    it "does not call #to_ary if it is private" do
      x = mock("multi-assign single RHS")
    ...
    it "raises a TypeError if #to_ary returns nil" do
      x = mock("multi-assign single RHS")
    ...
    it "does not call #to_ary to convert with a single splatted LHS" do
    ...
    it "raises a TypeError if #to_ary does not return an Array for a single RHS" do
      x = mock("multi-assign splat")
      x.should_receive(:to_ary).and_return(1)
  ...
  context "with a single splatted RHS value" do
    ...
    it "does not call #to_a to convert nil to an empty Array" do
    ...
    it "calls #to_a if #respond_to? returns false" do
      x = mock("multi-assign RHS splat")
      x.should_receive(:respond_to?).and_return(false)
      x.should_receive(:to_a).and_return([1, 2])
    ...
    it "calls #to_ary to convert an Object RHS with a single splat LHS" do
      x = mock("multi-assign RHS splat")
      x.should_receive(:to_ary).and_return([1, 2])
    ...
    it "calls #to_ary to convert an Object RHS with a single LHS" do
      x = mock("multi-assign RHS splat")
      x.should_receive(:to_ary).and_return([1, 2])
    ...
    it "calls #to_ary to convert an Object splat RHS when assigned to a simple MLHS" do
      x = mock("multi-assign splat")
      x.should_receive(:to_ary).and_return([1, 2])
  ...
  context "with a MRHS value" do
    ...
    it "calls #to_ary to convert a splatted Object as part of a MRHS with a splat MLHS" do
      x = mock("multi-assign splat MRHS")
      x.should_receive(:to_ary).and_return([2, 3])

      (a, *b = 1, *x).should == [1, 2, 3]
      [a, b].should == [1, [2, 3]]
    end
    ...
    it "does not call #to_ary to convert an Object when the position receiving the value is a rest variable" do
      ...
      a, b, c, *d = 1, 2, 3, x
      [a, b, c, d].should == [1, 2, 3, [x]]
    end

    it "does not call #to_ary on an Array if the position receiving the value is a rest variable" do
      x = [3, 4]
      x.should_not_receive(:to_ary)

      a, b, *c = 1, 2, *x
      [a, b, c].should == [1, 2, [3, 4]]
    end

    it "calls #to_ary to convert a splatted Object when the position receiving the value is a rest variable" do
      x = mock("multi-assign mixed splatted RHS")
      x.should_receive(:to_ary).and_return([3,4])
