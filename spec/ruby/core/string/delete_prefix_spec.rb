...
ruby_version_is '2.5' do
  ...
  describe "String#delete_prefix!" do
    ...
    it "raises a RuntimeError when self is frozen" do
      -> { 'hello'.freeze.delete_prefix!('hell') }.should raise_error(RuntimeError)
      -> { 'hello'.freeze.delete_prefix!('') }.should raise_error(RuntimeError)
      -> { ''.freeze.delete_prefix!('') }.should raise_error(RuntimeError)
    end
  end
end