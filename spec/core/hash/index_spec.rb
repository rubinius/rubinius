...
describe "Hash#index" do
  ...
  it "compares values with same order as keys() and values()" do
    ...
    h.values[0].should_receive(:==).with(needle).and_return(false)
    h.values[1].should_receive(:==).with(needle).and_return(false)
    h.values[2].should_receive(:==).with(needle).and_return(true)
