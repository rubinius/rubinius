...
describe "Bignum instance method" do
  ...
  it "+ should return self plus other" do
    a = S_BignumHelper.sbm(76)
    (a + 4).should == 1073741904
    (a + 4.2).should == 1073741904.2
    (a + S_BignumHelper.sbm(3)).should == 2147483727
  end
  
  it "- should return self minus other" do
    a = S_BignumHelper.sbm(314)
    (a - 9).should == 1073742129
    (a - 12.57).should == 1073742125.43
    (a - S_BignumHelper.sbm(42)).should == 272
  end
  
  it "- should negate self" do
    S_BignumHelper.sbm.send(:-@).should == (-1073741824)
    S_BignumHelper.sbm(921).send(:-@).should == (-1073742745)
  end
  ...
  it "/ should raise ZeroDivisionError if other is zero and not a Float" do
    should_raise(ZeroDivisionError) { S_BignumHelper.sbm / 0 }
  end
  ...
  it "<=> should return -1, 0, 1 when self is less than, equal, or greater than other" do
    a = S_BignumHelper.sbm(96)
    (a <=> S_BignumHelper.sbm(196)).should == -1
    (a <=> a).should == 0
    (a <=> 4.5).should == 1
  end
  
  it "== should true if self has the same value as other" do
    a = S_BignumHelper.sbm(67)
    (a == a).should == true
    (a == 5.4).should == false
    (a == 121).should == false
  end
  ...
  specify "== calls 'other == self' if coercion fails" do
    class X; def ==(other); S_BignumHelper.sbm(123) == other; end; end

    (S_BignumHelper.sbm(120) == X.new).should == false
    (S_BignumHelper.sbm(123) == X.new).should == true
  end
  
  it "> should return true if self is greater than other" do
    a = S_BignumHelper.sbm(732)
    (a > (a + 500)).should == false
    (a > 14.6).should == true
    (a > (S_BignumHelper.sbm - 1)).should == true
  end
  
  it ">= should return true if self is greater than or equal to other" do
    a = S_BignumHelper.sbm(14)
    (a >= a).should == true
    (a >= (a + 2)).should == false
  ...
  it "abs should return the absolute value" do
    S_BignumHelper.sbm(39).abs.should == 1073741863
    (-S_BignumHelper.sbm(18)).abs.should == 1073741842
  end
  
  it "<< should return self shifted left other bits" do
    a = S_BignumHelper.sbm(9)
  ...
  it "coerce should return [other, self] both as Bignum if other is an Integer" do
    ...
    a.coerce(S_BignumHelper.sbm(701)).should == [1073742525, 1073741824]
  end
  ...
  it "div should raise ZeroDivisionError if other is zero and not a Float" do
    should_raise(ZeroDivisionError) { S_BignumHelper.sbm.div(0) }
  end
  
  it "div should NOT raise ZeroDivisionError if other is zero and is a Float" do
    S_BignumHelper.sbm(5_221).div(0.0)#.to_s.should == 'Infinity'
    S_BignumHelper.sbm(25).div(-0.0)#.to_s.should == '-Infinity'
  end
  ...
  it "hash should be provided" do
    S_BignumHelper.sbm.respond_to?(:hash).should == true
  end
  ...
  it "% should raise ZeroDivisionError if other is zero and not a Float" do
    should_raise(ZeroDivisionError) { S_BignumHelper.sbm.modulo(0) }
  end
  ...
  it "quo should NOT raise an exception when other is zero" do
    ...
    # but fails here.
    a = S_BignumHelper.sbm(91)
    b = S_BignumHelper.sbm(28)
  ...
  it "remainder should raise ZeroDivisionError if other is zero and not a Float" do
    should_raise(ZeroDivisionError) { S_BignumHelper.sbm(66).remainder(0) }
  end
  
  it "remainder should NOT raise ZeroDivisionError if other is zero and is a Float" do
    a = S_BignumHelper.sbm(7)
    b = S_BignumHelper.sbm(32)
  ...
  it "| should return self bitwise OR other" do
    ...
    (a | S_BignumHelper.sbm).should == 1073741835
  end
  
  it "~ should return self bitwise inverted" do
    (~S_BignumHelper.sbm(48)).should == -1073741873
    (~(-S_BignumHelper.sbm(21))).should == 1073741844
    (~S_BignumHelper.sbm(1)).should == -1073741826
  end  
