...
shared :complex_math_atan2 do |obj|
  describe "Math#{obj == Math ? '.' : '#'}atan2" do
    it "returns the arc tangent of the passed arguments" do
      obj.send(:atan2, 4.2, 0.3).should be_close(1.49948886200961, TOLERANCE)
      obj.send(:atan2, 0.0, 1.0).should be_close(0.0, TOLERANCE)
      obj.send(:atan2, -9.1, 3.2).should be_close(-1.23265379809025, TOLERANCE)
      obj.send(:atan2, 7.22, -3.3).should be_close(1.99950888779256, TOLERANCE)
    end
...
shared :complex_math_atan2_bang do |obj|
  describe "Math#{obj == Math ? '.' : '#'}atan2!" do
    ...
    it "raises a TypeError when passed a Complex number" do
      lambda { obj.send(:atan2!, Complex(4, 5), Complex(4, 5)) }.should raise_error(TypeError)
      lambda { obj.send(:atan2!, 4, Complex(4, 5)) }.should raise_error(TypeError)
      lambda { obj.send(:atan2!, Complex(4, 5), 5) }.should raise_error(TypeError)
    end
  end
end