...
describe "Net::FTP#voidresp" do
  ...
  it "is private" do
    @ftp.private_methods.should include("voidresp")
  end
  
  it "reads one response from the socket and returns nil" do
    @socket.should_receive(:readline).and_return("2xx Does not raise.")
    @ftp.send(:voidresp).should be_nil
  end
  ...
  it "raises a Net::FTPReplyError when the response code is 1xx" do
    ...
    lambda { @ftp.send(:voidresp) }.should raise_error(Net::FTPReplyError)
  end
  ...
  it "raises a Net::FTPTempError when the response code is 4xx" do
    ...
    lambda { @ftp.send(:voidresp) }.should raise_error(Net::FTPTempError)
  end
  ...
  it "raises a Net::FTPPermError when the response code is 5xx" do
    ...
    lambda { @ftp.send(:voidresp) }.should raise_error(Net::FTPPermError)
  end
  ...
  it "raises a Net::FTPProtoError when the response code is not valid" do
    ...
    lambda { @ftp.send(:voidresp) }.should raise_error(Net::FTPProtoError)
  end
end
