...
context "The equal primitive sent the wrong number of args" do
  ...
  specify "should raise PrimitiveFailure with more than 2 args" do
    run_primitive(:equal, 1, 2, 3).should_raise(PrimitiveFailure)
    run_primitive(:equal, 1, 2, 3, 4).should_raise(PrimitiveFailure)
  end

  specify "should raise PrimitiveFailure with less than 2 args" do
    run_primitive(:equal, 1).should_raise(PrimitiveFailure)
    run_primitive(:equal).should_raise(PrimitiveFailure)
  end
end
