...
require "socket"
require "test/unit"
require "#{File.dirname(__FILE__)}/envutil"
...
class OpenSSL::TestSSL < Test::Unit::TestCase
  ...
  def server_loop(ctx, ssls, server_proc)
    loop do
      ...
      Thread.start(ssl) do |client|
        Thread.current.abort_on_exception = true
        server_proc.call(ctx, client)
      end
    end
  ...
  def start_server(port0, verify_mode, start_immediately, args = {}, &block)
    ...
    server_proc ||= proc { |a,b| readwrite_loop(a,b) } # method(:readwrite_loop)
    ...
    begin
      ...
      $stderr.printf("%s started: port=%d\n", SSL_SERVER, port) if $DEBUG
