...
namespace rubinius {
  ...
  CharArray* CharArray::create(STATE, native_int bytes) {
  ...
  CharArray* CharArray::create_pinned(STATE, native_int bytes) {
    assert(bytes >= 0 && bytes < INT32_MAX);

    size_t body = bytes;
    CharArray* ba = state->vm()->om->new_object_bytes_mature<CharArray>(state, G(chararray), body);
    if(unlikely(!ba)) {
      Exception::memory_error(state);
    }
  ...
  CharArray* CharArray::allocate(STATE, Fixnum* bytes) {
    return CharArray::create(state, bytes->to_native());
  }

  Fixnum* CharArray::size(STATE) {
    return Fixnum::from(size());
  }

  Fixnum* CharArray::get_byte(STATE, Fixnum* index) {
  ...
  Fixnum* CharArray::set_byte(STATE, Fixnum* index, Fixnum* value) {
    native_int idx = index->to_native();
  ...
  Fixnum* CharArray::move_bytes(STATE, Fixnum* start, Fixnum* count, Fixnum* dest) {
    ...
    if(src < 0) {
      ...
      Exception::object_bounds_exceeded_error(state, "move is beyond end of chararray");
  ...
  CharArray* CharArray::prepend(STATE, String* str) {
    CharArray* ba = CharArray::create(state, size() + str->size());
  ...
  CharArray* CharArray::fetch_bytes(STATE, Fixnum* start, Fixnum* count) {
    native_int src = start->to_native();
    native_int cnt = count->to_native();
    ...
    CharArray* ba = CharArray::create(state, cnt + 1);
  ...
  Object* CharArray::locate(STATE, String* pattern, Fixnum* start, Fixnum* max_o) {
  ...
  Object* CharArray::get_utf8_char(STATE, Fixnum* offset) {
