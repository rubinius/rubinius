...
class TestBignum : public CxxTest::TestSuite, public VMTest {
  ...
  void setUp() {
    create();
    b1 = Bignum::from(state, (intptr_t)2147483647);
    b2 = Bignum::from(state, (intptr_t)2147483646);
    two = Fixnum::from(2);
    TOLERANCE = 0.00003;
  }
  ...
  void test_add_positive_range() {
    ...
    Bignum* big = Bignum::from(state,(long long) FIXNUM_MAX+2);
  ...
  void test_add_negative_range() {
    Bignum* fix = Bignum::from(state,(long long) FIXNUM_MIN-1);
    Integer* sum = fix->add(state, Bignum::from(state, 1));
    ...
    fix = Bignum::from(state,(long long) FIXNUM_MIN-2);
    sum = fix->add(state, Bignum::from(state, 1));
  ...
  void test_add_with_positive_fixnum() {
    ...
    fix = Bignum::from(state,(long long) FIXNUM_MIN-1);
    ...
    big = Bignum::from(state,(long long) FIXNUM_MIN-2);
    sum = big->add(state, one);
  ...
  void test_add_with_float() {
    ...
    Float* a = Bignum::from(state, (intptr_t)FIXNUM_MAX + 10)->add(state, f);
  ...
  void test_sub_positive_range() {
    ...
    fix = Bignum::from(state,(long long) FIXNUM_MAX+2);
    diff = fix->sub(state, Bignum::from(state, 1));
  ...
  void test_sub_with_float() {
    Float* f = Float::create(state, 0.2);
    Float* a = Bignum::from(state, (intptr_t)FIXNUM_MAX + 10)->sub(state, f);
  ...
  void test_mul_negative_range() {
    Bignum* neg1 = Bignum::from(state, -1);
    Bignum* a = Bignum::from(state, FIXNUM_MAX + 1);
    Integer* b = a->mul(state, neg1);

    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MIN, b->to_native());

    a = Bignum::from(state, b->to_native());
    b = a->add(state, neg1);

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MIN - 1, b->to_native());

    a = Bignum::from(state, FIXNUM_MIN);
    b = a->mul(state, neg1);
    ...
    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX, b->to_native());
  }
  ...
  void test_mul_with_positive_fixnum() {
    ...
    Bignum* a = Bignum::from(state, FIXNUM_MIN);
    Integer* b = a->sub(state, one);
    ...
    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(((FIXNUM_MIN - 1) / 2 - 1), b->to_native());

    Bignum* c = Bignum::from(state, b->to_native());
    b = c->mul(state, two);

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MIN - 2, b->to_native());
  }
  ...
  void test_mul_with_negative_fixnum() {
    ...
    Integer* b = neg_half->mul(state, neg_two);

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX + 3, b->to_native());

    Bignum* half = Bignum::from(state, (FIXNUM_MAX + 1) / 2);
    b = half->mul(state, neg_two);

    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MIN, b->to_native());

    Fixnum* neg_third = as<Fixnum>(Bignum::from(state, FIXNUM_MIN)->div(state, Fixnum::from(3)));
    b = neg_third->mul(state, neg_three);

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX + 3, b->to_native());

    Fixnum* third = as<Fixnum>(Bignum::from(state, FIXNUM_MAX)->div(state, Fixnum::from(3)));
    b = third->mul(state, neg_three);
  ...
  void test_mul_with_float() {
    Float* f = Float::create(state, 0.2);
    Float* a = Bignum::from(state, (intptr_t)FIXNUM_MAX + 10)->mul(state, f);
  ...
  void test_div() {
    ...
    Bignum* big = Bignum::from(state,(unsigned long long) big_int * 2);
    Integer* b = big->div(state, two);

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX+1, b->to_native());

    b = big->div(state, neg_two);

    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MIN, b->to_native());

    big = Bignum::from(state,(long long) FIXNUM_MAX * 2);
    b = big->div(state, two);
    ...
    Bignum* zero = Bignum::from(state, (intptr_t)0);
  ...
  void test_mod() {
    Bignum* neg = Bignum::from(state,(long long) FIXNUM_MIN-2);
    Integer* b = neg->mod(state, Bignum::from(state,(long long) FIXNUM_MAX+1));

    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX - 1, b->to_native());

    Bignum* pos = Bignum::from(state,(long long) FIXNUM_MAX+2);
    b = pos->mod(state, Bignum::from(state,(long long) FIXNUM_MIN-1));

    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(0, b->to_native());

    neg = Bignum::from(state,(long long) FIXNUM_MIN-3);
    Integer* big = neg->mod(state, Bignum::from(state,(long long) FIXNUM_MAX+2));

    TS_ASSERT(kind_of<Fixnum>(big));
    TS_ASSERT_EQUALS(FIXNUM_MAX, big->to_native());

    pos = Bignum::from(state,(long long) FIXNUM_MAX+3);
    big = pos->mod(state, Bignum::from(state,(long long) FIXNUM_MIN-2));

    TS_ASSERT(kind_of<Fixnum>(big));
    TS_ASSERT_EQUALS(0, big->to_native());
  }

  void test_mod_with_fixnum() {
    Bignum* neg = Bignum::from(state, FIXNUM_MIN-1);
    Integer* b = neg->mod(state, Fixnum::from(FIXNUM_MAX));

    TS_ASSERT(kind_of<Fixnum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX-2, b->to_native());

    Bignum* pos = Bignum::from(state, FIXNUM_MAX+1);
    b = pos->mod(state, Fixnum::from(FIXNUM_MIN));
  ...
  void test_positive_left_shift_positive() {
    ...
    Integer* b = one->left_shift(state, max_width);
  ...
  void test_positive_left_shift_negative() {
    ...
    TS_ASSERT_EQUALS(1L << (FIXNUM_MAX_WIDTH-1), val->to_native());
  }
  ...
  void test_negative_left_shift_positive() {
    ...
    TS_ASSERT_EQUALS(FIXNUM_MIN * 2, val->to_native());
  }
  ...
  void test_negative_left_shift_negative() {
    ...
    TS_ASSERT(kind_of<Fixnum>(val));
    TS_ASSERT_EQUALS((((0UL - 1L) << (FIXNUM_MIN_WIDTH - 2)) - 1), val->to_native());
  }
  ...
  void test_positive_right_shift_negative() {
    ...
    Bignum* big = Bignum::from(state, 1L << (FIXNUM_MAX_WIDTH - 1));
  ...
  void test_negative_right_shift_negative() {
    Bignum* neg_one = Bignum::from(state, -1);
    Fixnum* neg_min_width = Fixnum::from(-(FIXNUM_MIN_WIDTH - 1));

    Integer* val = neg_one->right_shift(state, neg_min_width);

    TS_ASSERT(kind_of<Fixnum>(val));
    TS_ASSERT_EQUALS(FIXNUM_MIN, val->to_native());
  }
  ...
  void test_pow() {
    ...
    Fixnum* width_minus1 = Fixnum::from(FIXNUM_MAX_WIDTH-1);
    Fixnum* width = Fixnum::from(FIXNUM_MAX_WIDTH);

    Integer* fix = as<Integer>(two->pow(state, width_minus1));

    TS_ASSERT(kind_of<Fixnum>(fix));
    TS_ASSERT_EQUALS(1L << (FIXNUM_MAX_WIDTH-1), fix->to_native());

    fix = as<Integer>(neg_two->pow(state, width_minus1));

    TS_ASSERT(kind_of<Fixnum>(fix));
    TS_ASSERT_EQUALS((intptr_t)((0UL - 1L) << (FIXNUM_MAX_WIDTH-1)), fix->to_native());
    ...
    TS_ASSERT_EQUALS(1L << (FIXNUM_MAX_WIDTH), big->to_native());

    big = as<Integer>(neg_two->pow(state, Fixnum::from(FIXNUM_MAX_WIDTH+1)));

    TS_ASSERT(kind_of<Bignum>(big));
    TS_ASSERT_EQUALS((long long)((0ULL - 1LL) << (FIXNUM_MAX_WIDTH+1)), as<Bignum>(big)->to_long_long());
  }
  ...
  void test_compare_with_fixnum() {
    TS_ASSERT_EQUALS(Fixnum::from(0),
        Bignum::from(state, (intptr_t)3)->compare(state, Fixnum::from(3)));
    TS_ASSERT_EQUALS(Fixnum::from(1),
        Bignum::from(state, (intptr_t)3)->compare(state, Fixnum::from(2)));
    TS_ASSERT_EQUALS(Fixnum::from(-1),
        Bignum::from(state, (intptr_t)3)->compare(state, Fixnum::from(4)));
  ...
  void test_compare_with_float() {
    TS_ASSERT_EQUALS(Fixnum::from(0),
        Bignum::from(state, (intptr_t)3)->compare(state, Float::create(state, 3.0)));
    TS_ASSERT_EQUALS(Fixnum::from(1),
        Bignum::from(state, (intptr_t)3)->compare(state, Float::create(state, 2.9)));
    TS_ASSERT_EQUALS(Fixnum::from(-1),
        Bignum::from(state, (intptr_t)3)->compare(state, Float::create(state, 3.1)));
  ...
  void test_neg() {
    ...
    TS_ASSERT_EQUALS(FIXNUM_MIN, negated->to_native());
  ...
  void test_bit_and() {
    Bignum* pos = Bignum::from(state, (1L<<FIXNUM_MAX_WIDTH)|(1L<<(FIXNUM_MAX_WIDTH-1)));
    Integer* a = pos->bit_and(state, Bignum::from(state, FIXNUM_MAX));

    TS_ASSERT(kind_of<Fixnum>(a));
    TS_ASSERT_EQUALS(1L<<(FIXNUM_MAX_WIDTH-1), a->to_native());

    Bignum* neg = Bignum::from(state, FIXNUM_MIN);
    a = neg->bit_and(state, Bignum::from(state, FIXNUM_MIN));

    TS_ASSERT_EQUALS(FIXNUM_MIN, a->to_native());
    TS_ASSERT(kind_of<Fixnum>(a));
    ...
    neg = Bignum::from(state, FIXNUM_MIN);
    big = neg->bit_and(state, Bignum::from(state,(long long) FIXNUM_MIN-1));

    TS_ASSERT(kind_of<Bignum>(big));
    TS_ASSERT_EQUALS(1L << FIXNUM_MIN_WIDTH, big->to_native());
  }
  ...
  void test_bit_or() {
    ...
    Integer* a = pos->bit_or(state, Bignum::from(state, FIXNUM_MAX));

    TS_ASSERT(kind_of<Fixnum>(a));
    TS_ASSERT_EQUALS(FIXNUM_MAX, a->to_native());

    Bignum* neg = Bignum::from(state, FIXNUM_MIN);
    a = neg->bit_or(state, Bignum::from(state, FIXNUM_MIN));

    TS_ASSERT_EQUALS(FIXNUM_MIN, a->to_native());
    TS_ASSERT(kind_of<Fixnum>(a));

    Integer* b = pos->bit_or(state, Bignum::from(state, (FIXNUM_MAX+1)));

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS((1L<<FIXNUM_MAX_WIDTH)|1, b->to_native());

    neg = Bignum::from(state, FIXNUM_MIN);
    b = neg->bit_or(state, Bignum::from(state, FIXNUM_MIN-1));

    TS_ASSERT_EQUALS(-1, b->to_native());
    TS_ASSERT(kind_of<Fixnum>(b));
  }
  ...
  void test_bit_xor() {
    Bignum* pos = Bignum::from(state, 1);
    Integer* a = pos->bit_xor(state, Bignum::from(state, FIXNUM_MAX-1));

    TS_ASSERT(kind_of<Fixnum>(a));
    TS_ASSERT_EQUALS(FIXNUM_MAX, a->to_native());

    Bignum* neg = Bignum::from(state, 1);
    a = neg->bit_xor(state, Bignum::from(state,(long long) FIXNUM_MIN-1));

    TS_ASSERT(kind_of<Bignum>(a));
    TS_ASSERT_EQUALS(FIXNUM_MIN - 2, a->to_native());

    pos = Bignum::from(state, (FIXNUM_MAX+1)|FIXNUM_MAX);
    Integer* b = pos->bit_xor(state, Bignum::from(state, FIXNUM_MAX));

    TS_ASSERT(kind_of<Bignum>(b));
    TS_ASSERT_EQUALS(FIXNUM_MAX+1, b->to_native());

    neg = Bignum::from(state, FIXNUM_MAX);
    b = neg->bit_xor(state, Bignum::from(state, -1));
  ...
  void test_bit_xor_with_fixnum() {
    Bignum* pos = Bignum::from(state, 1);
    Integer* a = pos->bit_xor(state, Fixnum::from(FIXNUM_MAX-1));

    TS_ASSERT(kind_of<Fixnum>(a));
    TS_ASSERT_EQUALS(FIXNUM_MAX, a->to_native());

    Bignum* neg = Bignum::from(state,(long long) FIXNUM_MIN-1);
    a = neg->bit_xor(state, Fixnum::from(1));
  ...
  void test_gt() {
    ...
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->gt(state, Fixnum::from(-3)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->gt(state, Fixnum::from(-2)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->gt(state, Fixnum::from(-4)));

    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->gt(state, Float::create(state, -3.0)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->gt(state, Float::create(state, -2.9)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->gt(state, Float::create(state, -3.1)));
  }
  ...
  void test_ge() {
    ...
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->ge(state, Fixnum::from(-3)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->ge(state, Fixnum::from(-2)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->ge(state, Fixnum::from(-4)));

    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->ge(state, Float::create(state, -3.0)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->ge(state, Float::create(state, -2.9)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->ge(state, Float::create(state, -3.1)));
  }
  ...
  void test_lt() {
    ...
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->lt(state, Fixnum::from(-3)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->lt(state, Fixnum::from(-2)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->lt(state, Fixnum::from(-4)));

    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->lt(state, Float::create(state, -3.0)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->lt(state, Float::create(state, -2.9)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->lt(state, Float::create(state, -3.1)));
  }
  ...
  void test_le() {
    ...
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->le(state, Fixnum::from(-3)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->le(state, Fixnum::from(-2)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->le(state, Fixnum::from(-4)));

    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->le(state, Float::create(state, -3.0)));
    TS_ASSERT_EQUALS(cTrue,
        Bignum::from(state, (intptr_t)-3)->le(state, Float::create(state, -2.9)));
    TS_ASSERT_EQUALS(cFalse,
        Bignum::from(state, (intptr_t)-3)->le(state, Float::create(state, -3.1)));
  }
