/************************************************
  ...
  $Author: nobu $
  $Date: 2009-10-10 23:54:54 -0700 (Sat, 10 Oct 2009) $
  modified at: Mon Jan 24 15:59:52 JST 1994
  - Evan Phoenix (restyle, modernize) - 2010
...
fgdbm_initialize(int argc, VALUE* argv, VALUE obj) {
  ...
  if(!NIL_P(vflags)) flags = NUM2INT(vflags);
...
fgdbm_s_open(int argc, VALUE* argv, VALUE klass) {
  ...
  if(NIL_P(fgdbm_initialize(argc, argv, obj))) return Qnil;
...
fgdbm_fetch(VALUE obj, VALUE keystr, VALUE ifnone) {
  ...
  if(NIL_P(valstr)) {
    if(ifnone == Qnil && rb_block_given_p()) return rb_yield(keystr);
    return ifnone;
  }
  return valstr;
}
...
static VALUE
fgdbm_indexes(int argc, VALUE* argv, VALUE obj) {
  VALUE new;
  int i;

  rb_warn("GDBM#indexes is deprecated; use GDBM#values_at");
 ...
 */
static VALUE
fgdbm_select(int argc, VALUE* argv, VALUE obj) {
...
rb_gdbm_delete(VALUE obj, VALUE keystr) {
  ...
  GetDBM2(obj, dbmp, dbm);
  if(!gdbm_exists(dbm, key)) return Qnil;
...
fgdbm_has_key(VALUE obj, VALUE keystr) {
  ...
  GetDBM2(obj, dbmp, dbm);
  if(gdbm_exists(dbm, key)) return Qtrue;
  return Qfalse;
}
