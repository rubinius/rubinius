  ...
  def heredoc here # 63 lines
    ...
    if expand then
      ...
      until src.scan(eos_re) do
        ...
        if c != "\n" then
          self.yacc_value = token_buffer.join.delete("\r")
      ...
      if src.scan(/\ |\t|\r|\f|\13/) then # white spaces, 13 = '\v
        ...
        if src.scan(/\n|#/) then
          ...
          if [:expr_beg, :expr_fname,
            ...
            # @@stats[:case2_3] += 1 if SPY
            next
          end

          # @@stats[:case2_4] += 1 if SPY
          ...
          if src.scan(/(\$_)(\w)/) then
            ...
            if last_state == :expr_fname then
              ...
              self.yacc_value = token_buffer.join[1..-1].to_i
              return :tNTH_REF
            end
