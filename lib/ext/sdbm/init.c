/************************************************
  ...
  $Author: knu $
  $Date: 2008-11-10 01:18:23 -0800 (Mon, 10 Nov 2008) $
    ...
    VALUE obj;
{
  ...
  Data_Get_Struct(obj, struct dbmdata, dbmp);
  if(dbmp == 0) return Qtrue;
  if(dbmp->di_dbm == 0) return Qtrue;
    ...
    VALUE klass;
{
  ...
  if(NIL_P(fsdbm_initialize(argc, argv, obj))) return Qnil;
    ...
    VALUE obj;
{
  ...
  rb_warn("SDBM#indexes is deprecated; use SDBM#values_at");
