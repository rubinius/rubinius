...
describe "Process.wait" do
  ...
  it "waits for a specific child if a pid is given" do
    pid1 = Process.fork { sleep(0.5); Process.exit! }
  ...
  it "waits for a child whose process group ID is that of the calling process" do
    pid1 = Process.fork { Process.setpgid(0, 0); Process.exit! }
    Process.setpgid(0, 0)
    ppid = Process.pid
    pid2 = Process.fork {
      Process.setpgid(0, ppid);
      sleep(0.1); Process.exit!
    }
    sleep(0.1); # to give children a chance to set their process groups
